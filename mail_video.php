...
$sql = "SELECT m.ID AS `mid`, d.id AS dnr, MAKE_MOVIE_TITLE( `m`.`title`, `m`.`comment`, `s`.`name`, `es`.`episode`, `s`.`prepend`, `m`.`omu` ) AS `title`, ".
  "duration_string(m.duration) AS dur, IF( `l`.`name` IS NOT NULL, TRIM( GROUP_CONCAT( `l`.`name` ORDER BY `ml`.`lang_id` DESC SEPARATOR ', ' ) ), 'n. V.' ) AS `lingos`, ".
  "d.name AS disc, AVG(ur.rating) AS avg_rating, m.omdb_id AS oid FROM movies AS m LEFT JOIN disc AS d ON m.disc = d.id LEFT JOIN user_ratings AS ur ON ur.movie_id = m.id ".
  "LEFT JOIN movie_languages AS ml ON ml.movie_id = m.ID LEFT JOIN languages AS l ON l.id = ml.lang_id LEFT JOIN `episode_series` AS `es` ON `m`.`ID` = `es`.`movie_id` ".
  "LEFT JOIN `series` AS `s` ON `s`.`id` = `es`.`series_id` WHERE m.ID = ".$_GET['mid'];

$result = MySQLBase::instance()->con()->query($sql);
$rows   = $result->fetch_assoc();

$ch = curl_init($r->getLink()."/omdb.php?cover-oid=".$rows['oid']);
...
mail($_GET['mailto'], 
  "Schrott- bzw. Rentnerfilm: #".$rows['mid']." - ".$rows['title'],
  $mail, 
  "From: Heikos Schrott- & Rentnerfilme <heiko@rangun.de>\nContent-Type: text/html; charset=utf-8");
